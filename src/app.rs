...
fn run_diff_app(
    ...
    loop {
        terminal.draw(|frame| {
            ...
            render_diff_toolbar(frame, layout.toolbar, title, &theme);
            ...
            render_diff_status_bar(frame, layout.status, &status, &stats_str, &theme);
...
fn render_diff_toolbar(
    ...
    let spans = vec![
        ...
        Span::styled(
            format!("  {} ", title),
            Style::new().fg(theme.toolbar_fg).bg(theme.toolbar_bg),
        ),
    ];
...
fn render_diff_status_bar(
    frame: &mut ratatui::Frame,
    area: Rect,
    status: &crate::views::StatusInfo,
    stats_str: &str,
    theme: &Theme,
) {
    let left = format!(" {} ", status.cursor_path);
    let right_width = crate::util::display_width(stats_str);
    ...
    let spans = vec![
        ...
        Span::styled(" ".repeat(padding), Style::new().bg(theme.status_bg)),
        Span::styled(
            stats_str.to_string(),
